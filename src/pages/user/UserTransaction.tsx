  ...
  return (
    <div className='mt-3 py-4 bg-white flex rounded-md justify-between items-center'>
      ...
      <div className="flex justify-end space-x-5 mr-10  items-center">
        <Link to={`/user/transaction/${id}`} className="bg-sky-600 text-white text-xs px-3 py-2 rounded-md font-bold inline-block">Detail</Link>
      </div>
    </div>
  )
}
const UserTransaction: React.FC = () => {
  ...
  useEffect(()=>{
    (async()=>{
      let t = await useService(TransactionService.fetchTransactionByUser(),nav,undefined,undefined,true,undefined,false)
  ...
  return (
    <>
    <div className="pl-28 top-20 absolute">Transaction History</div>
